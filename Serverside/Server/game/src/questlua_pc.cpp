...
namespace quest 
{
	...
	int pc_give_item_from_special_item_group(lua_State* L)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (dwVnums[i] == 1)
					ch->ChatPacket(CHAT_TYPE_INFO, "Nu ai primit nimic.", dwCounts[i]);
				else if (dwVnums[i] == 2)
					ch->ChatPacket(CHAT_TYPE_INFO, "Ai primit %d puncte de experien��.", dwCounts[i]);
			}
		}
		return 0;
	}
	...
	int pc_set_level(lua_State * L)  
	{
		...
		else
		{
			...
			ch->PointChange(POINT_STAT, ((MINMAX(1, newLevel, 99) - ch->GetLevel()) * 3) + ch->GetPoint(POINT_LEVEL_STEP));
			//레벨
	...
	int pc_give_exp_perc(lua_State* L)
	{
		...
		DWORD exp = (DWORD)((exp_table_common[MINMAX(0, lev, 99)] * proc) / 100);
		PC * pPC = CQuestManager::instance().GetCurrentPC();
