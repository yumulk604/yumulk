...
OSVERSIONINFOEX CIME::ms_stOSVI;
...
bool CIME::Initialize(HWND hWnd)
{
	...
	ms_stOSVI.dwOSVersionInfoSize = sizeof(OSVERSIONINFOEX);
	...
	if (VerifyVersionInfoA(&ms_stOSVI, VER_MAJORVERSION | VER_MINORVERSION | VER_PLATFORMID | VER_SERVICEPACKMAJOR | VER_SERVICEPACKMINOR, dwlConditionMask))
		bUnicodeImm = true;
...
void CIME::ChangeInputLanguage()
{
	...
	if (uLanguage != GETLANG())
	{
		...
		SetSupportLevel(ms_dwIMELevelSaved);
	}
...
void CIME::CandidateProcess(HIMC hImc)
{
	...
	if(dwCandidateLen > 0) {
		...
		int iStartOfPage = ms_bUILessMode ? lpCandidateList->dwPageStart : (ms_dwCandidateSelection / (MAX_CANDLIST - 1)) * (MAX_CANDLIST - 1);
				...
				case IMEID_CHS_VER42: // 4.2.x.x // SCIME98 or MSPY2 (w/Office2k, Win2k, WinME, etc)
					{
						...
						int op = VER_GREATER_EQUAL;
						VER_SET_CONDITION( dwlConditionMask, VER_PLATFORMID, op );
						VerifyVersionInfoA(&osi, VER_PLATFORMID, dwlConditionMask);
