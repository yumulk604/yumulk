...
bool CShopManager::StartShopping(LPCHARACTER pkChr, LPCHARACTER pkChrShopKeeper, int iShopVnum)
{
	...
	{
		pkChr->ChatPacket(CHAT_TYPE_INFO, "Altceva se petrece 頽 acest moment.");
		return false;
	}
...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "E簍i prea departe.");
		return;
	}
	...
	if (SHOP_SUBHEADER_GC_OK != ret) // 巩力啊 乐菌栏搁 焊辰促.
	{
		TPacketGCShop pack;
...
void CShopManager::Sell(LPCHARACTER ch, BYTE bCell, BYTE bCount)
{
	...
	if (item->IsEquipped())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Nu pot vinde ceva ce folosesc.");
		return;
	}
	...
	if (iVal > 0 && dwPrice > 0)
		ch->ChatPacket(CHAT_TYPE_INFO, "V鈔zarea a fost cu %d%% impozitat�.", iVal);
