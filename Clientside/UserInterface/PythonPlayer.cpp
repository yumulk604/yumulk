...
{
	return __GetLevelAtk()+(__GetStatAtk()+__GetWeaponAtk(dwWeaponPower+dwRefineBonus))*__GetHitRate()/100;
}

DWORD CPythonPlayer::__GetHitRate()
{
	return (min(90, (GetStatus(POINT_DX) * 4 + GetStatus(POINT_LEVEL) * 2)/6)+210)/3;
}
...
void CPythonPlayer::SendClickItemPacket(DWORD dwIID)
{
	...
	if (strlen(c_szOwnerName) > 0 && strcmp(c_szOwnerName, GetName()))
	{
		CItemData * pItemData;
