...
void CPrivManager::GiveGuildPriv(DWORD guild_id, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	if (g)
	{
		...
		{
			char buf[100];
			snprintf(buf, sizeof(buf), "%s a breslei %s este crescutã la %d%%.", GetPrivName(type), g->GetName(), value);
			SendNotice(buf);
		}
		else
		{
			char buf[100];
			snprintf(buf, sizeof(buf), "%s a breslei %s a revenit la normal.", GetPrivName(type), g->GetName());
			SendNotice(buf);
		}
...
void CPrivManager::GiveEmpirePriv(BYTE empire, BYTE type, int value, BYTE bLog, time_t end_time_sec)
{
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), "%s a regatului %s este crescutã la %d%%.", GetPrivName(type), GetEmpireName(empire), value);
	...
	{
		char buf[100];
		snprintf(buf, sizeof(buf), "%s a regatului %s a revenit la normal.", GetPrivName(type), GetEmpireName(empire));
