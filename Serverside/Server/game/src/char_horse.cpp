...
bool CHARACTER::StartRiding()
{
	if (IsDead())
	{
		ChatPacket(CHAT_TYPE_INFO, "Nu poþi cãlãri.");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Nu poþi cãlãri cu aceastã îmbrãcãminte.");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Nu te poþi urca pe cal cu o undiþã în mânã.");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Nu te poþi urca pe cal cu un târnãcop în mânã.");
		return false;
	}
	...
	{
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, "Nivelul calului este prea scãzut.");
		else if (GetHorseHealth() <= 0)
			ChatPacket(CHAT_TYPE_INFO, "Punctele de viaþã ale calului sunt prea scãzute.");
		else if (GetHorseStamina() <= 0)
			ChatPacket(CHAT_TYPE_INFO, "Stamina calului este prea scãzutã.");
...
void CHARACTER::HorseSummon(bool bSummon, bool bFromFar, DWORD dwVnum, const char* pPetName)
{
	if ( bSummon )
	{
		...
		if (!m_chHorse)
		{
			ChatPacket(CHAT_TYPE_INFO, "Calul nu a fost chemat.");
			return;
		}
