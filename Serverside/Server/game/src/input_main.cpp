...
void CInputMain::GoldCoinAction(LPCHARACTER ch, const char* c_pData)
{
    ...
    switch (p->bSubHeader)
    {
        case GOLD_COIN_SUBHEADER_CG_DEPOSIT:
        {
            ...
            item->SetSocket(0, 1); // Madeni Para olduÄŸunu belirtmek iÃ§in
            ...
            ch->ChatPacket(CHAT_TYPE_INFO, "%d Yang, Madeni Paraya dÃ¶nÃ¼ÅŸtÃ¼rÃ¼ldÃ¼.", p->iValue);
            break;
        }
        case GOLD_COIN_SUBHEADER_CG_WITHDRAW:
        {
            // Bu kÄ±sÄ±m daha sonra implemente edilecek.
            break;
        }
    }
}
