...
void MessengerManager::P2PLogin(keyA account)
{
	Login(account);
}

void MessengerManager::P2PLogout(keyA account)
{
	Logout(account);
}

void MessengerManager::Login(keyA account)
{
...
void MessengerManager::RequestToAdd(LPCHARACTER ch, LPCHARACTER target)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, "Cibirichi46");
	    return;
	}
...
bool MessengerManager::AuthToAdd(keyA account, keyA companion, bool bDeny)
{
...
void MessengerManager::__AddToList(keyA account, keyA companion)
{
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, "%s a fost adãugat la lista de prieteni.", companion.c_str());
...
void MessengerManager::AddToList(keyA account, keyA companion)
{
	if (companion.size() == 0)
		return;
	...
	DBManager::instance().Query("INSERT INTO messenger_list VALUES ('%s', '%s')", 
...
void MessengerManager::__RemoveFromList(keyA account, keyA companion)
{
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, "%s a fost eliminat din lista de prieteni.", companion.c_str());
...
void MessengerManager::RemoveFromList(keyA account, keyA companion)
{
	if (companion.size() == 0)
		return;
	...
	DBManager::instance().Query("DELETE FROM messenger_list WHERE account='%s' AND companion = '%s'",
			account.c_str(), companionEscaped);
...
void MessengerManager::SendList(keyA account)
{
	LPCHARACTER ch = CHARACTER_MANAGER::instance().FindPC(account.c_str());
...
void MessengerManager::SendLogin(keyA account, keyA companion)
{
	LPCHARACTER ch = CHARACTER_MANAGER::instance().FindPC(account.c_str());
	LPDESC d = ch ? ch->GetDesc() : NULL;
...
void MessengerManager::SendLogout(keyA account, keyA companion)
{
	if (!companion.size())
		return;
