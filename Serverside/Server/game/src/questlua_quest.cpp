...
namespace quest
{
	...
	int quest_coroutine_yield(lua_State * L)
	{
		...
		if (q.IsInOtherPCBlock())
		{
			...
			if (NULL == pQS || NULL == q.GetQuestStateName(stQuestName, pQS->st))
			{
				sys_err("	... WHO AM I? WHERE AM I? I only know QuestName(%s)...", stQuestName.c_str());
			}
			else
			{
				sys_err("	Current Quest(%s). State(%s)", stQuestName.c_str(), q.GetQuestStateName(stQuestName, pQS->st));
			}
			return 0;
		}
