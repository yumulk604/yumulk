...
{
	char szQuery[128];
	snprintf(szQuery, sizeof(szQuery), "SHOW CREATE TABLE player");
...
bool PlayerHB::Query(DWORD id)
{
	...
	char szTableName[64];
	snprintf(szTableName, sizeof(szTableName), "hb_%02d%02d%02d%02d_player", 
			curr_tm.tm_year - 100, curr_tm.tm_mon + 1, curr_tm.tm_mday, curr_tm.tm_hour);
	...
	{
		char szFind[32];
		snprintf(szFind, sizeof(szFind), "CREATE TABLE `player`");
	...
	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO %s SELECT * FROM player.player WHERE id=%u", m_stTableName.c_str(), id);
