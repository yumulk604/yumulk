...
void CPythonMiniMap::Update(float fCenterX, float fCenterY)
{
	...
	for(i = rkChrMgr.CharacterInstanceBegin(); i!=rkChrMgr.CharacterInstanceEnd(); ++i)
	{
		...
		float fXWhiteMark = (m_fWidth - (float)m_WhiteMark.GetWidth()) / 2.0f + fDistanceFromCenterX + m_fScreenX;
		float fYWhiteMark = (m_fHeight - (float)m_WhiteMark.GetHeight()) / 2.0f + fDistanceFromCenterY + m_fScreenY;
		...
		if (pkInstEach->IsPC() && !pkInstEach->IsInvisibility())
		{
			...
			aMarkPosition.m_fX = fXWhiteMark;
			aMarkPosition.m_fY = fYWhiteMark;
	...
	std::sort(m_MinimapPosVector.begin(), m_MinimapPosVector.end(), [](TMarkPosition a, TMarkPosition b){ return a.m_bType > b.m_bType; });
...
bool CPythonMiniMap::LoadAtlas()
{
	...
	_snprintf(atlasFileName, sizeof(atlasFileName), "%s/atlas.sub", rkMap.GetName().c_str());	
	if (!CEterPackManager::Instance().isExist(atlasFileName))	
		_snprintf(atlasFileName, sizeof(atlasFileName), "d:/ymir work/ui/atlas/%s/atlas.sub", rkMap.GetName().c_str());
...
void CPythonMiniMap::__RenderMiniWayPointMark(int ixCenter, int iyCenter)
{
	...
	rInstance.SetPosition(ixCenter - rInstance.GetWidth()/2, iyCenter - rInstance.GetHeight()/2 + 2);
	rInstance.Render();
}
...
void CPythonMiniMap::__RenderTargetMark(int ixCenter, int iyCenter)
{
	...
	rInstance.SetPosition(ixCenter - rInstance.GetWidth()/2 + 20, iyCenter - rInstance.GetHeight()/2 + 20);
	rInstance.Render();
}
