					...
					if (pkAff)
					{
								...
								m_pkChr->GetPoint(POINT_STAT) != pkAff->lApplyValue)
						{
							...
							//if (pMap->IsFlagOnBase(idx))
							{
								...
								char buf[256];
								snprintf(buf, sizeof(buf), "Cibirichi26 %s %s", pMap->GetGuild(idx)->GetName(), pMap->GetGuild(idx_opp)->GetName());
								pMap->Notice(buf);
							}
						}
					}
...
void CHARACTER::StateFlag()
{
	...
	snprintf(buf, sizeof(buf), "%s a capturat steagul de la %s.", pMap->GetGuild(idx)->GetName(), f.m_pkChrFind->GetName());
	pMap->Notice(buf);
}
...
void CHARACTER::StateHorse()
{
	...
	if (fDist >= START_FOLLOW_DISTANCE)
	{
		if (fDist > START_RUN_DISTANCE)
			SetNowWalking(false);		// NOTE: 함수 이름보고 멈추는건줄 알았는데 SetNowWalking(false) 하면 뛰는거임.. -_-;

		Follow(victim, number(MIN_APPROACH, MAX_APPROACH));

		m_dwStateDuration = STATE_DURATION;
	}
	else if (get_dword_time() > m_dwLastAttackTime)
	{
		// wondering-.-
