...
namespace quest
{
	...
	int npc_get_leader_vid(lua_State* L)
	{
		...
		const LPPARTY pkParty = (npc) ? npc->GetParty() : NULL;
		const LPCHARACTER pkChrLeader = (pkParty) ? pkParty->GetLeader() : NULL;

		lua_pushnumber(L, (pkChrLeader) ? pkChrLeader->GetVID() : 0);
		return 1;
	}
