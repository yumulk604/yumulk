...
LPCHARACTER CHARACTER_MANAGER::SpawnMob(DWORD dwVnum, long lMapIndex, long x, long y, long z, bool bSpawnMotion, int iRot, bool bShow)
{
	...
	if (!pkMob)
	{
		sys_err("SpawnMob: no mob data for vnum %u mapindex %d at %d %d", dwVnum, lMapIndex, x, y);
		return NULL;
	}
	...
	if (!sectree)
	{
		sys_log(0, "SpawnMob: cannot create monster at non-exist sectree %d x %d (map %d) %d", x, y, lMapIndex, dwVnum);
		return NULL;
	}
