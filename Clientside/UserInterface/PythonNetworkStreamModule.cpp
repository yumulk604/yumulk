...
PyObject* netGetEmpireCountByID(PyObject* poSelf, PyObject* poArgs)
{
	int empireID;
	if (!PyTuple_GetInteger(poArgs, 0, &empireID))
		return Py_BuildException();

	CPythonNetworkStream& rkNetStream=CPythonNetworkStream::Instance();
	return Py_BuildValue("i", rkNetStream.GetEmpireCountByID(empireID));
}
...
PyObject* netSendPartyRemovePacket(PyObject* poSelf, PyObject* poArgs)
{
	...
	int isVid;
	if (!PyTuple_GetInteger(poArgs, 1, &isVid))
		return Py_BuildException();
	...
	if (isVid)
	{
		...
		rns.SendPartyRemovePacket(dwVID);
...
void initnet()
{
	static PyMethodDef s_methods[] =
	{
		...
		{ "GetEmpireCountByID",					netGetEmpireCountByID,					METH_VARARGS },
