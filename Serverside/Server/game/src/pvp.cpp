...
void CPVPManager::Insert(LPCHARACTER pkChr, LPCHARACTER pkVictim)
{
	...
	if (pkPVP)
	{
		...
		{
			pkVictim->ChatPacket(CHAT_TYPE_INFO, "Duelul cu %s a început.", pkChr->GetName());
			pkChr->ChatPacket(CHAT_TYPE_INFO, "Duelul cu %s a început.", pkVictim->GetName());
		}
		return;
	}
	...
	snprintf(msg, sizeof(msg), "%s te-a provocat la duel.", pkChr->GetName());

	pkVictim->ChatPacket(CHAT_TYPE_INFO, msg);
	pkChr->ChatPacket(CHAT_TYPE_INFO, "%s a fost provocat la duel.", pkVictim->GetName());
