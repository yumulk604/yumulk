...
DWORD CGuildManager::CreateGuild(TGuildCreateParameter& gcp)
{
	...
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, "<Breaslã> ªir necorespunzãtor.");
		return 0;
	}
	...
	if (pmsg->Get()->uiNumRows > 0)
	{
		...
		if (!(row[0] && row[0][0] == '0'))
		{
			gcp.master->ChatPacket(CHAT_TYPE_INFO, "<Breaslã> ªir existent.");
			return 0;
		}
	}
	else
	{
		gcp.master->ChatPacket(CHAT_TYPE_INFO, "<Breaslã> Nu pot face asta.");
		return 0;
	}
...
void CGuildManager::DeclareWar(DWORD guild_id1, DWORD guild_id2, BYTE bType)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), "Breasla %s a declarat rãzboi breslei %s.", TouchGuild(guild_id1)->GetName(), TouchGuild(guild_id2)->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::RefuseWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1 && g2)
	{
		if (g2->GetMasterCharacter())
			g2->GetMasterCharacter()->ChatPacket(CHAT_TYPE_INFO, "Breasla %s a refuzat declaraþia de rãzboi.", g1->GetName());
	}
...
void CGuildManager::WaitStartWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	{
		char buf[256];
		snprintf(buf, sizeof(buf), "Rãzboiul dintre %s ºi %s a început.", g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void SendGuildWarOverNotice(CGuild* g1, CGuild* g2, bool bDraw)
{
	if (g1 && g2)
	{
		...
		if (bDraw)
		{
			snprintf(buf, sizeof(buf), "Razboiul dintre %s ºi %s s-a terminat la egalitate.", g1->GetName(), g2->GetName());
		}
		else
		{
			if (g1->GetWarScoreAgainstTo(g2->GetID()) > g2->GetWarScoreAgainstTo(g1->GetID()))
				snprintf(buf, sizeof(buf), "Breasla %s a câºtigat rãzboiul împotriva breslei %s.", g1->GetName(), g2->GetName());
			else
				snprintf(buf, sizeof(buf), "Breasla %s a câºtigat rãzboiul împotriva breslei %s.", g2->GetName(), g1->GetName());
		}
...
void CGuildManager::CancelWar(DWORD guild_id1, DWORD guild_id2)
{
	...
	if (g1)
	{
		...
		if (master1)
			master1->ChatPacket(CHAT_TYPE_INFO, "Rãzboiul a fost anulat.");
	}
	...
	if (g2)
	{
		...
		if (master2)
			master2->ChatPacket(CHAT_TYPE_INFO, "Rãzboiul a fost anulat.");
	}

	if (g1 && g2)
	{
		char buf[256+1];
		snprintf(buf, sizeof(buf), "Rãzboiul dintre %s ºi %s a fost anulat.", g1->GetName(), g2->GetName());
		SendNotice(buf);
	}
}
...
void CGuildManager::ShowGuildWarList(LPCHARACTER ch)
{
	for (itertype(m_GuildWar) it = m_GuildWar.begin(); it != m_GuildWar.end(); ++it)
	{
		...
		if (A && B)
			ch->ChatPacket(CHAT_TYPE_NOTICE, "%s[%d] vs %s[%d] time %u sec.", A->GetName(), A->GetID(), B->GetName(), B->GetID(), get_global_time() - A->GetWarStartTime(B->GetID()));
	}
}
