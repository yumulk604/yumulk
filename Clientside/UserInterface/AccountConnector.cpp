...
string GetMachineGUID2()
{
	string ret = "not windows";
	char value[64];
	DWORD size = _countof(value), type = REG_SZ;
	HKEY key;
	if (::RegOpenKeyExA(HKEY_LOCAL_MACHINE, "SOFTWARE\\Microsoft\\Cryptography", 0, KEY_READ | KEY_WOW64_64KEY, &key) == ERROR_SUCCESS && 
		::RegQueryValueExA(key, "MachineGuid", nullptr, &type, (LPBYTE)value, &size) == ERROR_SUCCESS)
	{
		::RegCloseKey(key);
		ret = string(value, size);
	}
	return ret;
}
...
bool CAccountConnector::__AuthState_RecvPhase()
{
	...
	else if (kPacketPhase.phase == PHASE_AUTH)
	{
		...
		strncpy_s(LoginPacket.guid, GetMachineGUID2().c_str(), GUID_MAX_NUM);
		LoginPacket.name[ID_MAX_NUM] = '\0';
		LoginPacket.pwd[PASS_MAX_NUM] = '\0';
		LoginPacket.guid[GUID_MAX_NUM] = '\0';

		LoginPacket.version = 1280;
