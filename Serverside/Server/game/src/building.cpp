...
CObject::CObject(TObject * pData, TObjectProto * pProto) : m_pProto(pProto), m_dwVID(0), m_chNPC(NULL)
{
...
void CObject::RegenNPC()
{
	...
	CGuild* pGuild = CGuildManager::instance().FindGuild(m_pkLand->GetOwner());
	if (!pGuild)
		return;
...
struct FIsIn
{
	...
	bool bIn;
	FIsIn (	long sx_, long sy_, long ex_, long ey_) : sx(sx_), sy(sy_), ex(ex_), ey(ey_), bIn(false) {}
	...
	void operator () (LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (sx <= ch->GetX() && ch->GetX() <= ex && sy <= ch->GetY() && ch->GetY() <= ey)
				bIn = true;
		}
	}
};
...
bool CManager::LoadObjectProto(const TObjectProto * pProto, int size) // from DB
{
	...
	for (int i = 0; i < size; ++i)
	{
		...
		for (int j = 0; j < OBJECT_MATERIAL_MAX_NUM; ++j)
		{
			...
			if (!ITEM_MANAGER::instance().GetTable(r.kMaterials[j].dwItemVnum))
			{
