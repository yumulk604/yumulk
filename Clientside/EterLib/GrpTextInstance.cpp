...
int CGraphicTextInstance::Hyperlink_GetText(char* buf, int len)
{
	...
	return Ymir_WideCharToMultiByte(1250, 0, gs_hyperlinkText.c_str(), gs_hyperlinkText.length(), buf, len, NULL, NULL);	
}
...
void CGraphicTextInstance::Update()
{
	...
	std::unique_ptr<wchar_t[]> wText(new wchar_t[wTextMax]);
	...
	while (begin < end)
	{
		...
		int wTextLen = Ymir_MultiByteToWideChar(dataCodePage, 0, begin, token - begin, wText.get(), wTextMax);
