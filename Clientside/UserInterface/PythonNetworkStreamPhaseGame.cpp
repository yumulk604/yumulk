		...
		case SHOP_SUBHEADER_GC_START_EX:
			{
				...
				for (size_t i = 0; i < shop_tab_count; i++)
				{
					...
					CPythonShop::instance().SetTabCoinType(static_cast<BYTE>(i), pPackTab->coin_type);
					CPythonShop::instance().SetTabName(static_cast<BYTE>(i), pPackTab->name);
					...
					for (BYTE j = 0; j < SHOP_HOST_ITEM_MAX_NUM; j++)
					{
						...
						CPythonShop::Instance().SetItemData(static_cast<BYTE>(i), j, *itemData);
					}
				}
...
bool CPythonNetworkStream::SendBuildOfflineShopPacket(const char * c_szName, const std::vector<TShopItemTable> & c_rSellingItemStock)
{
    ...
    packet.bHeader = HEADER_CG_OFFLINE_SHOP;
    strncpy(packet.szSign, c_szName, SHOP_SIGN_MAX_LEN);
    packet.bCount = static_cast<BYTE>(c_rSellingItemStock.size());
    if (!Send(sizeof(packet), &packet))
        return false;
...
bool CPythonNetworkStream::SendClientVersionPacket()
{
	...
	strncpy(kVersionPacket.timestamp, "1215955206", sizeof(kVersionPacket.timestamp)-1);
