...
#define SIEGE_EVENT_PULSE	PASSES_PER_SEC(60*5)	// 5ºÐ
...
EVENTFUNC(castle_siege_event)
{
	...
	{
		snprintf(buf, sizeof(buf), "%s Cibirichi13", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
		BroadcastNotice(buf);
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_STRUGGLE:
			{
				snprintf(buf, sizeof(buf), "%s Cibirichi14", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);

				snprintf(buf, sizeof(buf), "%s Cibirichi15", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
				BroadcastNotice(buf);
...
EVENTFUNC(castle_stone_event)
{
	...
	if (info->spawn_count < (SPAWN_COUNT * 2))
	{
		...
		if (info->spawn_count < (SPAWN_COUNT * 2))
			return PASSES_PER_SEC(30 * 60);	// 30ºÐ
		else
			return 0;
	}
...
void castle_start_siege(int empire, int tower_count)
{
	...
	/* °ø¼º Å¸ÀÌ¸Ó ½ÃÀÛ */
	{
		...
		GET_SIEGE_EVENT(empire) = event_create(castle_siege_event, info, /*5ºÐ*/SIEGE_EVENT_PULSE);
	}
...
bool castle_spawn_tower(int empire, int tower_count)
{
	...
	char buf[1024];
	snprintf(buf, sizeof(buf), "%s marcheazã începutul rãzboiului.", EMPIRE_NAME(empire));
	BroadcastNotice(buf);
...
/* È²±Ý µÎ²¨ºñ°¡ Á×À¸¸é killer¿¡°Ô 1Ãµ¸¸³É */
...
void castle_tower_die(LPCHARACTER ch, LPCHARACTER killer)
{
	...
	switch (GET_SIEGE_STATE())
	{
		...
		case CASTLE_SIEGE_END:
			{
				int	siege_end = true;
				snprintf(buf, sizeof(buf), "%s Cibirichi17", EMPIRE_NAME(killer_empire));
				BroadcastNotice(buf);
				...
				if (siege_end)
				{
					...
					{
						snprintf(buf, sizeof(buf), "Regatul %s a pierdut rãzboiul.", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					else
					{
						snprintf(buf, sizeof(buf), "Regatul %s a distrus toate obiectivele.", EMPIRE_NAME(GET_SIEGE_EMPIRE()));
						BroadcastNotice(buf);
					}
					castle_end_siege();
				}
