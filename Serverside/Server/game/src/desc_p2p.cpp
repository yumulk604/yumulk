...
bool DESC_P2P::Setup(LPFDWATCH fdw, socket_t fd, const char * host, WORD wPort)
{
	...
	{
		sys_log(0, "SYSTEM: BLOCKED new p2p connection from [%s] to [%s] fd: %d", host, g_szPublicIP, m_sock);
	...
	sys_log(0, "SYSTEM: ACCEPTED new p2p connection from [%s] fd: %d", host, m_sock);
	return true;
}
