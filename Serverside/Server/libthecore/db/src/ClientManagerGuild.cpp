...
void CClientManager::GuildAddMember(CPeer* peer, TPacketGDGuildAddMember * p)
{
	...
	snprintf(szQuery, sizeof(szQuery), 
			"INSERT INTO guild_member VALUES(%u, %u, %d, 0, 0)",
...
void CClientManager::GuildRemoveMember(CPeer* peer, TPacketGuild* p)
{
	...
	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO quest (dwPID, szName, szState, lValue) VALUES(%u, 'guild_manage', 'new_withdraw_time', %u)", p->dwInfo, (DWORD) GetCurrentTime());
	CDBManager::instance().AsyncQuery(szQuery);
...
void CClientManager::GuildDisband(CPeer* peer, TPacketGuild* p)
{
	...
	snprintf(szQuery, sizeof(szQuery), "REPLACE INTO quest (dwPID, szName, szState, lValue) SELECT pid, 'guild_manage', 'new_disband_time', %u FROM guild_member WHERE guild_id = %u", (DWORD) GetCurrentTime(), p->dwGuild);
	CDBManager::instance().AsyncQuery(szQuery);
