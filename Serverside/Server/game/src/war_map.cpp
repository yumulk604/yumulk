...
{
	// set_pidJoiner ¥¬ ¥©¿˚ ¿Œø¯¿ª ∞ËªÍ«œ±‚ ∂ßπÆø° ¡¶∞≈«œ¡ˆ æ ¥¬¥Ÿ
	--iMemberCount;
}
...
void CWarMap::IncMember(LPCHARACTER ch)
{
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Œn acest moment participi ca spectator la r„zboi.");
	}
...
void CWarMap::CheckWarEnd()
{
	...
	if (m_TeamData[0].iMemberCount == 0 || m_TeamData[1].iMemberCount == 0)
	{
		...
		Notice("Daca niciun membru din breasla advers„ nu particip„, r„zboiul se sf‚r∫e∫te Óntr-un minut.");
