	...
	while (it != m_memberMap.end())
	{
		...
		if (rMember.pCharacter)
		{
			if (rMember.pCharacter->GetDesc())
			{
				...
				rMember.pCharacter->ChatPacket(CHAT_TYPE_INFO, "Grupa a fost desfinþatã.");
			}
			else
			{
...
void CParty::SummonToLeader(DWORD pid)
{
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, "Jucãtorul nu a fost gãsit.");
		return;
	}
	...
	{
		l->ChatPacket(CHAT_TYPE_INFO, "Nu pot face asta.");
		return;
	}
	...
	if (n == 0)
		l->ChatPacket(CHAT_TYPE_INFO, "Acel loc nu e pentru mine.");
	else
	{
		int i = number(0, n - 1);
...
void CParty::Update()
{
	...
	for (it = m_memberMap.begin(); it != m_memberMap.end(); ++it)
	{
		...
		if (bLongTimeExpBonusChanged && ch->GetDesc())
			ch->ChatPacket(CHAT_TYPE_INFO, "Grupul a primit un bonus de experienþã datoritã bunei cooperãri.");
	...
	if (!m_bPartyHealReady)
	{
		...
		if (m_bCanUsePartyHeal && get_dword_time() > m_dwPartyHealTime + PartyHealCoolTime)
		{
