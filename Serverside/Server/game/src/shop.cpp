...
int CShop::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	if (m_pkPC)
	{
		...
		if (iVal > 0 && dwPrice > 0)
			m_pkPC->ChatPacket(CHAT_TYPE_INFO, "Cump�rarea a fost cu %d%% impozitat�.", iVal);
	}
	else
	{
		item->AddToCharacter(ch, TItemPos(INVENTORY, iEmptyPos));
...
long CShop::SystemAutoBuy(DWORD desiredVnum, BYTE desiredCount, long maxUnitPrice, DWORD ownerPID)
{
    if (!IsPCShop())
        return 0;
    ...
    {
        SHOP_ITEM & r_item = m_itemVector[i];
        if (!r_item.pkItem)
            continue;
        if (r_item.vnum != desiredVnum)
            continue;
        if (desiredCount != 0 && r_item.count != desiredCount)
            continue;
        if (r_item.price <= 0)
            continue;

        long unit = r_item.count > 0 ? (r_item.price / r_item.count) : r_item.price;
        if (unit > maxUnitPrice)
            continue;
        ...
        // Log as SYSTEM_AUTOBUY before destroy
        LogManager::instance().ItemLog(m_pkPC, item, "SYSTEM_AUTOBUY", item->GetName());

        // Remove item from seller inventory and mark sold out
        item->RemoveFromCharacter();
        ITEM_MANAGER::instance().FlushDelayedSave(item);
        r_item.pkItem = NULL;
        BroadcastUpdateItem(i);

        // Deposit gold to seller's safebox (by PID -> account_id)
        {
            char szQuery[256];
            snprintf(szQuery, sizeof(szQuery),
                    "UPDATE safebox s JOIN player p ON p.account_id=s.account_id SET s.gold = s.gold + %ld WHERE p.id=%u",
                    r_item.price, m_pkPC->GetPlayerID());
            DBManager::instance().DirectQuery(szQuery);
        }

        // Money log (spent from system perspective)
        DBManager::instance().SendMoneyLog(MONEY_LOG_SHOP, item->GetVnum(), -r_item.price);

        // Destroy item object
        M2_DESTROY_ITEM(item);

        long paid = r_item.price;
        r_item.vnum = 0;
        r_item.count = 0;
        r_item.price = 0;
