...
DWORD g_GoldDropTimeLimitValue = 30;
	...
	void CQuestManager::QuestInfo(unsigned int pc, unsigned int quest_index)
	{
		...
		if ((pPC = GetPC(pc)))
		{
			...
			if (!CheckQuestLoaded(pPC))
			{
				...
				if (ch)
					ch->ChatPacket(CHAT_TYPE_INFO, "Aºteaptã un pic.");
