...
class SelectCharacterWindow(ui.Window):
	...
	def timp_frumos(self, minute):
		...
		if minute  <= 60:
			...
			if self.ore <= 24:
				return "%so si %sm" % (self.ore, minute - 60 * self.ore)
			else:
				self.zile = self.ore / 24
				return "%sz si %so" % (self.zile, self.ore - 24 * self.zile)
	...
	def OnUpdate(self):
		chr.Update()
		for i in xrange(4):
			self.NameList[i].SetAlpha(self.destNameAlpha[i])
			self.GaugeList[i].SetPercentage(self.destGauge[i], 1.0)
