...
bool CHARACTER::LearnGrandMasterSkill(DWORD dwSkillVnum)
{
	...
	{
		if (GetSkillMasterType(dwSkillVnum) > SKILL_GRAND_MASTER)
			ChatPacket(CHAT_TYPE_INFO, "Limita maximã a fost atinsã deja.");
		else
			ChatPacket(CHAT_TYPE_INFO, "Limita maximã a fost atinsã.");
		return false;
	}
	...
	{
		ChatPacket(CHAT_TYPE_TALKING, "Cartea nu a putut fi invãþatã.");
		ChatPacket(CHAT_TYPE_INFO, "Încercarea ta de a creºte aceastã competenþã a eºuat.");
		LogManager::instance().CharLog(this, dwSkillVnum, "GM_READ_FAIL", "");
		return false;
	}

	ChatPacket(CHAT_TYPE_TALKING, "Abilitate invãþatã cu succes.");
	ChatPacket(CHAT_TYPE_TALKING, "Abilitatea ta a crescut la un alt nivel.");
	ChatPacket(CHAT_TYPE_INFO, "Ai reuºit sã creºti aceastã competenþã la un nivel mai mare.");
...
bool CHARACTER::LearnSkillByBook(DWORD dwSkillVnum, BYTE bProb)
{
	...
	if (!IsLearnableSkill(dwSkillVnum))
	{
		ChatPacket(CHAT_TYPE_INFO, "Nu poþi învaþa aceastã competenþã.");
		return false;
	}
	...
	if ( GetExp() < need_exp )
	{
		ChatPacket(CHAT_TYPE_INFO, "Nu ai destulã experienþã pentru a citi aceastã carte.");
		return false;
	}
	...
	if (pkSk->dwType != 0)
	{
		...
		{
			if (GetSkillMasterType(dwSkillVnum) > SKILL_MASTER)
				ChatPacket(CHAT_TYPE_INFO, "Aceastã competenþã nu are nevoie de o carte pentru a fi antrenatã.");
			else
				ChatPacket(CHAT_TYPE_INFO, "Nu poþ citi aceastã carte.");
			return false;
		}
	}
		...
		if (pPC)
		{
			...
			if (number(1, 100) > percent)
			{
				...
				if (read_count >= need_bookcount)
				{
					...
					ChatPacket(CHAT_TYPE_INFO, "Antrenamentul a fost încheiat cu succes.");
				...
				else
				{
					...
					{
						case 1:
							ChatPacket(CHAT_TYPE_TALKING, "Am înþeles cât de cât învãþãtura...");
							break;
											
						case 2:
							ChatPacket(CHAT_TYPE_TALKING, "M-am pregãtit destul de bine pentur asta.");
							break;

						case 3:
						default:
							ChatPacket(CHAT_TYPE_TALKING, "Am reuºit.");
							break;
					}

					ChatPacket(CHAT_TYPE_INFO, "Am citit cartea cu succes. %d cãrþi de competenþã rãmase.", need_bookcount - read_count);
					return true;
				}
			}
		}
	...
	void operator () (LPENTITY ent)
	{
		...
		switch (m_pkSk->bSkillAttrType)
		{
			...
			case SKILL_ATTR_TYPE_MAGIC:
				...
				if (m_pkChr->GetJob() == JOB_SURA || m_pkChr->GetJob() == JOB_SHAMAN)
					iDam = abs(iDam * (85 - pkChrVictim->GetPoint(POINT_RESIST_MAGIC)) / 100);
				else
...
bool CHARACTER::UseSkill(DWORD dwVnum, LPCHARACTER pkVictim, bool bUseGrandMaster)
{
	...
	if (dwVnum == SKILL_HORSE_SUMMON)
	{
		...
		if (GetHorseLevel() <= 0)
			ChatPacket(CHAT_TYPE_INFO, "Calul tãu este mort.");
		else
			ChatPacket(CHAT_TYPE_INFO, "Cibirichi25");
...
{
	if (ms < 3 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "~ 97%");
	else if (ms < 5 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "~ 95%");
	else if (ms < 10 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "~ 90%");
	else if (ms < 30 * 60)
		ChatPacket(CHAT_TYPE_TALKING, "~ 75%");
	else if (ms < 1 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "~ 60%");
	else if (ms < 2 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "~ 45%");
	else if (ms < 3 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "~ 30%");
	else if (ms < 6 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "~ 10%");
	else if (ms < 12 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "~ 5%");
	else if (ms < 18 * 3600)
		ChatPacket(CHAT_TYPE_TALKING, "~ 2%");
	else
		ChatPacket(CHAT_TYPE_TALKING, "~ 0%");
}
...
bool CHARACTER::ResetOneSkill(DWORD dwVnum)
{
	...
	ChatPacket(CHAT_TYPE_INFO, "Ai primit %d puncte de abilitate înapoi.", level);
