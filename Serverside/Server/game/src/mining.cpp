...
namespace mining
{
	...
	void PracticePick(LPCHARACTER ch, LPITEM item)
	{
		...
		if (Pick_IsPracticeSuccess(pick))
		{
			if (Pick_Refinable(pick))
				ch->ChatPacket(CHAT_TYPE_INFO, "Târnãcopul poate fi acum îmbunãtãþit.");
			else
			{
				Pick_IncCurExp(pick);	

				ch->ChatPacket(CHAT_TYPE_INFO, "Punctele au crescut (%d/%d).",
	...
	EVENTFUNC(mining_event)
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Am nevoie de un târnãcop.");
			return 0;
		}
		// END_OF_REFINE_PICK
		...
		if (!load)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Nimic exploatabil împrejur.");
			return 0;
		}
		...
		if (number(1, 100) <= iPct)
		{
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "Explorare reuºitã.");
		}
		else
			ch->ChatPacket(CHAT_TYPE_INFO, "Explorare eºuatã.");
