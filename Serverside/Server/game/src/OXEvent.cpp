...
bool COXEventManager::ShowQuizList(LPCHARACTER pkChar)
{
	...
	for (size_t i = 0; i < m_vec_quiz.size(); ++i)
	{
		...
		{
			pkChar->ChatPacket(CHAT_TYPE_INFO, "%d %s %s", m_vec_quiz[i][j].level, m_vec_quiz[i][j].Quiz, m_vec_quiz[i][j].answer ? "Da" : "Nu");
		}
	}

	pkChar->ChatPacket(CHAT_TYPE_INFO, "Întrebãri: %d.", c);	
	return true;
}
...
EVENTFUNC(oxevent_timer)
{
	...
	{
		case 0:
			SendNoticeMap("10 secunde pânã la rezultat.", OXEVENT_MAP_INDEX, true);
			flag++;
		...
		case 1:
			if (info->answer == true)
			{
				...
				SendNoticeMap("O ^_^", OXEVENT_MAP_INDEX, true);
			}
			else
			{
				COXEventManager::instance().CheckAnswer(false);
				SendNoticeMap("X ^_^", OXEVENT_MAP_INDEX, true);
			}

			SendNoticeMap("Cei care au rãspuns greºit vor fi eliminaþi imediat.", OXEVENT_MAP_INDEX, true);
		...
		case 2:
			...
			SendNoticeMap("În curând va fi pusã urmãtoarea întrebare.", OXEVENT_MAP_INDEX, true);
			flag = 0;
			break;
	}
	return 0;
...
bool COXEventManager::Quiz(unsigned char level, int timelimit)
{
	...
	SendNoticeMap("O - adevãrat | X - fals", OXEVENT_MAP_INDEX, true);
...
bool COXEventManager::CheckAnswer(bool answer)
{
	...
	for (; iter != m_map_attender.end();)
	{
		...
		if (pkChar != NULL)
		{
			...
			{
				pkChar->ChatPacket(CHAT_TYPE_INFO, "Yeeey ^_^");
