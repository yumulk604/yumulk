...
namespace quest
{
	...
	int highscore_show(lua_State* L)
	{
		...
		DBManager::instance().ReturnQuery(QID_HIGHSCORE_SHOW, mypid, NULL, 
				"SELECT h.pid, p.name, h.value FROM highscore as h, player as p WHERE h.board = '%s' AND h.pid = p.id ORDER BY h.value %s LIMIT 10",
	...
	int highscore_register(lua_State* L)
	{
		...
		DBManager::instance().ReturnQuery(QID_HIGHSCORE_REGISTER, qi->dwPID, qi, 
				"SELECT value FROM highscore WHERE board='%s' AND pid=%u", qi->szBoard, qi->dwPID);
		return 1;
	}
	...
	int member_chat(lua_State* L)
	{
		...
		CQuestManager::Instance().GetCurrentPartyMember()->ChatPacket(CHAT_TYPE_TALKING, s.str().c_str());
		return 0;
	}
