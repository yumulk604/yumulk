...
static void _send_bonus_info(LPCHARACTER ch)
{
	...
	if (item_drop_bonus)
		ch->ChatPacket(CHAT_TYPE_NOTICE, "Rata de drop obiecte a fost majoratã la %d%%.", item_drop_bonus);

	if (gold_drop_bonus)
		ch->ChatPacket(CHAT_TYPE_NOTICE, "Rata de drop Yang a fost majoratã la %d%%.", gold_drop_bonus);

	if (gold10_drop_bonus)
		ch->ChatPacket(CHAT_TYPE_NOTICE, "Rata dublã de Yang a fost majoratã la %d%%.", gold10_drop_bonus);

	if (exp_bonus)
		ch->ChatPacket(CHAT_TYPE_NOTICE, "Rata de experienþã a fost majoratã la %d%%.", exp_bonus);
}
...
bool NewPlayerTable2(TPlayerTable * table, const char * name, BYTE race, BYTE shape, BYTE bEmpire)
{
	...
	table->hp		= JobInitialPoints[job].max_hp + table->ht * JobInitialPoints[job].hp_per_ht + table->sRandomHP;
	table->sp		= JobInitialPoints[job].max_sp + table->iq * JobInitialPoints[job].sp_per_iq + table->sRandomSP;
...
void CInputLogin::CharacterCreate(LPDESC d, const char * data)
{
	...
	if (!check_name(pinfo->name) || pinfo->shape > 1 || strlen(pinfo->name) > 12)
	{
		d->Packet(&packFailure, sizeof(packFailure));
		return;
	}
	...
	if (!strcmp(c_rAccountTable.login, pinfo->name))
	{
		TPacketGCCreateFailure pack;
		pack.header = HEADER_GC_CHARACTER_CREATE_FAILURE;
		pack.bType = 1;
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (ch->IsGM() && quest::CQuestManager::instance().GetEventFlag("invisibility.gm_observer_mode"))
	...
	if (!ch->GetQuestFlag("obiectedestart.sursa"))
	{
		ch->AutoGiveItem(27002, 200);
		ch->AutoGiveItem(27005, 200);
		ch->AutoGiveItem(70038, 200);
			...
			case JOB_ASSASSIN:
				{
					...
					if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
						M2_DESTROY_ITEM(item);
					ch->AutoGiveItem(19, 1);
					ch->AutoGiveItem(2009, 1);
					ch->AutoGiveItem(8000, 200);
				}
				break;
			case JOB_SURA:
				{
		...
		ch->SetQuestFlag("obiectedestart.sursa", 1);
	}
	...
	else
	{
		if (version != date)
		{
			ch->ChatPacket(CHAT_TYPE_NOTICE, "Versiunea clientului nu este compatibilã.");
			d->DelayedDisconnect(10);
