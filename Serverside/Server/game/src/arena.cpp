...
{
	return !(m_StartPointA.x == startA_X && m_StartPointA.y == startA_Y && m_StartPointB.x == startB_X && m_StartPointB.y == startB_Y);
}
...
void CArenaMap::SendArenaMapListTo(LPCHARACTER pChar, DWORD mapIdx)
{
	...
	for (; iter != m_listArena.end(); ++iter)
		pChar->ChatPacket(CHAT_TYPE_INFO, "ArenaMapInfo Map: %d stA(%d, %d) stB(%d, %d)", mapIdx, (CArena*)(*iter)->GetStartPointA().x, (CArena*)(*iter)->GetStartPointA().y, (CArena*)(*iter)->GetStartPointB().x, (CArena*)(*iter)->GetStartPointB().y);
}
...
EVENTFUNC(ready_to_start_event)
{
	...
	if (!chA || !chB)
	{
		...
		if (chA)
		{
			chA->ChatPacket(CHAT_TYPE_INFO, "Pentru a iesi trebuie sa te deconectezi.");
			sys_log(0, "ARENA: Oppernent is disappered. MyPID(%d) OppPID(%d)", pArena->GetPlayerAPID(), pArena->GetPlayerBPID());
		}

		if (chB)
		{
			chB->ChatPacket(CHAT_TYPE_INFO, "Pentru a iesi trebuie sa te deconectezi.");
	...
	switch (info->state)
	{
		case 0:
			{
				...
				if (count > 10000)
				{
					chA->ChatPacket(CHAT_TYPE_INFO, "Nu poti crea potiunea.");
					chB->ChatPacket(CHAT_TYPE_INFO, "Nu poti crea potiunea.");
				}
				else
				{
					...
					chA->ChatPacket(CHAT_TYPE_INFO, "%d potiuni maxim.", chA->GetPotionLimit());
					chB->ChatPacket(CHAT_TYPE_INFO, "%d potiuni maxim.", chB->GetPotionLimit());
				}
				chA->ChatPacket(CHAT_TYPE_INFO, "Asteapta 10 secunde pana ce lupta va incepe.");
				chB->ChatPacket(CHAT_TYPE_INFO, "Asteapta 10 secunde pana ce lupta va incepe.");
		...
		default:
			{
				chA->ChatPacket(CHAT_TYPE_INFO, "Lupta a fost incheiata din cauza unor probleme.");
				chB->ChatPacket(CHAT_TYPE_INFO, "Lupta a fost incheiata din cauza unor probleme.");
...
EVENTFUNC(duel_time_out)
{
	...
	else
	{
		...
		{
			case 0:
				chA->ChatPacket(CHAT_TYPE_NOTICE, "Asteapta 10 secunde pana ce lupta va incepe.");
				chB->ChatPacket(CHAT_TYPE_NOTICE, "Asteapta 10 secunde pana ce lupta va incepe.");
...
bool CArena::OnDead(DWORD dwPIDA, DWORD dwPIDB)
{
	...
	if (!pCharA && !pCharB)
		restart = false;
	else if (!pCharA && pCharB)
	{
		pCharB->ChatPacket(CHAT_TYPE_NOTICE, "Lupta a fost incheiata din cauza unor probleme.");
		restart = false;
	}
	else if (pCharA && !pCharB)
	{
		pCharA->ChatPacket(CHAT_TYPE_NOTICE, "Lupta a fost incheiata din cauza unor probleme.");
		restart = false;
	}
	else if (pCharA && pCharB)
	{
		if (m_dwPIDA == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s a castigat lupta.", pCharA->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s a castigat lupta.", pCharA->GetName());
				sys_log(0, "ARENA: Duel is end. Winner %s(%d) Loser %s(%d)", pCharA->GetName(), GetPlayerAPID(), pCharB->GetName(), GetPlayerBPID());
			}
			else
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, "%s a castigat runda.", pCharA->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, "%s a castigat runda.", pCharA->GetName());
		...
		else if (m_dwPIDB == dwPIDA)
		{
			...
			{
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s a castigat lupta.", pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s a castigat lupta.", pCharB->GetName());
				sys_log(0, "ARENA: Duel is end. Winner(%d) Loser(%d)", GetPlayerBPID(), GetPlayerAPID());
			}
			else
			{
				restart = true;
				pCharA->ChatPacket(CHAT_TYPE_INFO, "%s a castigat runda.", pCharB->GetName());
				pCharA->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());

				pCharB->ChatPacket(CHAT_TYPE_INFO, "%s a castigat runda.", pCharB->GetName());
				pCharB->ChatPacket(CHAT_TYPE_NOTICE, "%s %d : %d %s", pCharA->GetName(), m_dwSetPointOfA, m_dwSetPointOfB, pCharB->GetName());
	...
	{
		if (pCharA)
			pCharA->ChatPacket(CHAT_TYPE_INFO, "Asteapta 10 secunde pana ce lupta va incepe.");

		if (pCharB)
			pCharB->ChatPacket(CHAT_TYPE_INFO, "Asteapta 10 secunde pana ce lupta va incepe.");
...
void CArena::OnDisconnect(DWORD pid)
{
	...
	{
		if (GetPlayerB())
			GetPlayerB()->ChatPacket(CHAT_TYPE_INFO, "Celalalt jucator nu poate fi gasit.");
	...
	{
		if (GetPlayerA())
			GetPlayerA()->ChatPacket(CHAT_TYPE_INFO, "Celalalt jucator nu poate fi gasit.");
