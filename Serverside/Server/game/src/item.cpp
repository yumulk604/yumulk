...
bool CItem::EquipTo(LPCHARACTER ch, BYTE bWearCell)
{
	...
	if (this->GetVnum() == UNIQUE_ITEM_HIDE_ALIGNMENT_TITLE)
		m_pOwner->ShowAlignment(false);

	if (((this->GetWearFlag() & WEARABLE_UNIQUE && this->IsEquipped()) || !(this->GetWearFlag() & WEARABLE_UNIQUE)) && this->GetValue(0))
		this->StartUniqueExpireEvent();
...
bool CItem::HasOwnership() const
{
	return m_pkOwnershipEvent != NULL;
}
...
void CItem::AccessorySocketDegrade()
{
	if (GetAccessorySocketGrade() > 0)
	{
		...
		if (ch)
			ch->ChatPacket(CHAT_TYPE_INFO, "Finisajul de pe %s a disprut brusc.", GetName());
