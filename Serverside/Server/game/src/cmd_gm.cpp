...
ACMD(do_makeguild)
{
	...
	gm.CreateGuild(cp);
	ch->ChatPacket(CHAT_TYPE_INFO, "Breasla %s a fost fondat�.", cp.name);
}
...
ACMD(do_guild_state)
{
	...
	if (pGuild)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "%s nu exist�.", arg1);
}
...
ACMD(do_priv_guild)
{
	...
	if (*arg1)
	{
		...
		if (!g)
			ch->ChatPacket(CHAT_TYPE_INFO, "Aceast� breasl� nu exist�.");
		else
		{
			char buf[1024+1];
...
ACMD(do_ban_guid)
{
	...
	{
		if (ch)
			ch->ChatPacket(CHAT_TYPE_INFO, "Folose�te: /ban_guid <nume>");
...
ACMD(do_maintenance)
{
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "/maintenance <0 - f�r� mentenan�� | cu mentenan�� - 1>");
		return;
	}
...
ACMD(do_block_chat)
{
	...
	if (!tch)
	{
		...
		if (ch)
			ch->ChatPacket(CHAT_TYPE_INFO, "Opera�iune reu�it�.");
	...
	if (tch && ch != tch)
	{
		...
		{
			char buf[1024];
			snprintf(buf, sizeof(buf), "Juc�torului %s i-a fost blocat chatul pentru %ld minute.", arg1, lBlockDuration / 60);
			BroadcastNotice(buf);
		}
	}
}
		...
		case 'c':
			{
				...
				if (!t)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Nu s-au g�sit construc�ii.");
					return;
				}
				...
				if (t->dwDependOnGroupVnum)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "E�ec.");
						return;
					}
				}
				...
				if (GMLevel == GM_PLAYER)
				{
					...
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "Pre� dep�it.");
						return;
					}
...
ACMD(do_horse_level)
{
	...
	if (NULL == victim)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Caracterul nu exist�.");
		return;
	}
...
ACMD(do_end_duel)
{
	...
	if (!CArenaManager::instance().EndDuel(pChar->GetPlayerID()))
		ch->ChatPacket(CHAT_TYPE_INFO, "Duelul nu s-a �ncheiat cu succes.");
	else
		ch->ChatPacket(CHAT_TYPE_INFO, "Duelul s-a �ncheiat cu succes.");
}
...
ACMD(do_duel)
{
	...
	if (pChar1 != NULL && pChar2 != NULL)
	{
		...
		if (pParty != NULL)
		{
			...
			{
				pChar1->ChatPacket(CHAT_TYPE_INFO, "Ai fost eliminat din grup�.");
		...
		if (pParty != NULL)
		{
			...
			{
				pChar2->ChatPacket(CHAT_TYPE_INFO, "Ai fost eliminat din grup�.");
				pParty->Quit(pChar2->GetPlayerID());
			}
		}
		
		if (CArenaManager::instance().StartDuel(pChar1, pChar2, set, minute))
			ch->ChatPacket(CHAT_TYPE_INFO, "Lupta a �nceput.");
		else
			ch->ChatPacket(CHAT_TYPE_INFO, "Lupta nu a �nceput.");
	}
	else
		ch->ChatPacket(CHAT_TYPE_INFO, "Cineva nu este g�sit.");
}
...
ACMD(do_break_marriage)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "Ai divor�at.", pids.pid1, pids.pid2);
