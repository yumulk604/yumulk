...
int BlueDragon_StateBattle(LPCHARACTER pChar)
{
	...
	for (int i = 0 ; i < SkillCount ; ++i)
	{
		...
		{
			int SkillUsingDuration = static_cast<int>(CMotionManager::instance().GetMotionDuration( pChar->GetRaceNum(), MAKE_MOTION_KEY(MOTION_MODE_GENERAL, MOTION_SPECIAL_1 + SkillIndex) ));
...
int BlueDragon_Damage(LPCHARACTER me, LPCHARACTER pAttacker, int dam)
{
	...
	if (me->IsStone() && pAttacker->GetMountVnum())
		for (int i = 1 ; i <= 4 ; ++i)
